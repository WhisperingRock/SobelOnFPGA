	...
	\chapter{FPGA IP}
		...
		\section{Convolution}
			...
			\subsection{Design}
				...
				\textbf{Always}: Compute the dot product between matrices
				\[
					G = \sum_{i=0}^{8} m[i] * k[i]
				\] 

				\textbf{CLK}: Process result and clip
				\[
					\text{\texttt{Box Blur Result}} = 
					\begin{cases*}
						\text{\texttt{prev Result}} & , if $ \overline{\text{\texttt{MATRIX\_VALID}}}$ \\\\
						\begin{cases*}
							0 	& , if	$G_b < 0$ 	\\
							255	& , if	$G_b > 255$	\\
							\frac{G_b}{9} & , else
				...
				\]
				\[
					\text{\texttt{Sobel Result}} = 
					\begin{cases*}
						\text{\texttt{prev Result}} & , if $ \overline{\text{\texttt{MATRIX\_VALID}}}$ \\\\
						\begin{cases*}
							0 	& , if	$(G_x^2 + G_y^2) < p_{thresh}$ \\
							255	& , else
						\end{cases*} & , if $\text{\texttt{MATRIX\_VALID}}$
					\end{cases*}
				\]
				...
				It should be noted that the pixel threshold $p_{thresh}$ is currently set
				to 4,000 of 65,025. This threshold value was used rather than the
				euclidean distance $\sqrt{G_x^2 + G_y^2}$. Our tradeoff makes this 
				easier to compute s.t. we dont use sqrt but some of our edges 
				may be removed and lines may become discontinuous. For a
				proof of concept, this is acceptable. 
...
\hl{todo} : I really need to use UVM encapsulation/sanitization for testing this module. 
		...
		\section{Image Control Top}
			...
			\subsection{Vivado Generated Components}
...
\hl{todo}			
			...
			\subsection{Testing}
				...
				A visual comparison of the sim I/O \ref{testcode:ipt} was 
				performed with a grayscale \texttt{.bmp} of Lena (a), and 
				the results of the box blur were (b) and sobel (e). For 
				verification, the box blur image was subtracted from the 
				input and recorded in image (c). This image is suspicious 
				because it
				exhibits extreme areas of pixel change despite
				the averaging' blur filter. We arrive that image (b) 
				must be shifted over some due to the convolution process. 
				Image (b) also has a thin black line on the top and right 
				side, supporting this hypothesis. Shifting the output image (b)
				using \ref{fig:offset} and recalculating the difference between 
				input and output, we get image (d) which correctly illustrates 
				a uniform removal of high-freq data from the photo. 

				Analyzing the sobel photo (e), the strong lines are well
				defined but weaker ones lose their continuity. This is part
				of the tradeoff mentioned earlier as well as a drawback 
				from the filter itself\ldots I didn't claim sobel to be the 
				best edge detector\ldots
				...
				\begin{figure}[H]
					...
					\begin{subfigure}{0.4\textwidth}
						...
						\caption{Blur Difference Image (shift unfixed)}
					\end{subfigure}
					%\hfill
					\begin{subfigure}{0.4\textwidth}
						\centering
						...
						\caption{Blur Difference Image (shift fixed)}
					\end{subfigure}
					\begin{subfigure}{0.4\textwidth}
						\centering
						\includegraphics[
							width			= \textwidth,
							keepaspectratio	= true
						]{../01_supporting/image_control_unit_top/sobel_testimg1.png}
						\caption{Output Sobel Image}
					\end{subfigure}
			...
			Code : 
			\begin{itemize}
				\item \texttt{ImageProcessTop.sv} \ref{code:ipt}
				\item \texttt{ImageProcessTop\_tb.sv} \ref{testcode:ipt}
			\end{itemize}
	...
	\chapter{Appendix}
		\section{SystemVerilog}
			...
			\subsection{Image Process Top}
	
				\lstinputlisting[
					language	= {Verilog},
					captionpos	= top,
					caption		= {\texttt{ImageProcessTop.sv}},
					breaklines	= true,
					label		= {code:ipt}
				]{../02_hardware/src/ImageProcessTop.sv}

				\lstinputlisting[
					language	= {Verilog},
					captionpos	= top,
					caption		= {\texttt{ImageProcessTop\_tb.sv}},
					breaklines	= true,
					label		= {testcode:ipt}
				]{../02_hardware/sim/ImageProcessTop_tb.sv}
